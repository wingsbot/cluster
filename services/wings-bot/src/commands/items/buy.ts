...
export default class Buy extends CommandBase {
  ...
  exec = async ({ interaction, responder, options }: CommandData) => {
    ...
    if (shopItem.maxInInv && existingItem && existingItem.quantity >= shopItem.maxInInv) {
      responder.error(`You can't have more than ${shopItem.maxInInv.toLocaleString()} **${shopItem.name}**!`, true);
      return;
    }

    if (shopItem.itemId === 'upgradebank') shopItem.price = BigInt(Math.round(Number(userData.bankCap) / 3));
    ...
    if (shopItem.itemId === 'upgradebank') {
      const newBankSpace = BigInt(Math.round(Number(userData.bankCap) / 4));
    ...
    this.client.modules.economy.addInventoryItem(interaction.member.id, Object.assign(shopItem, { userId: interaction.member.id }));
    responder.success(`Successfully purchased **x${shopItem.quantity} ${shopItem.name}**`);
  };
}
