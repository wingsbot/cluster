...
export default class Fish extends CommandBase {
  ...
  private fishing() {
    ...
    if (randomNumber <= 0.5) {
      ...
      const price = BigInt(this.client.util.getRandomInt(5, 20));
    ...
    if (randomNumber <= 0.89) {
      ...
      const price = BigInt(this.client.util.getRandomInt(100 + rarity.value, 250 + rarity.value));
      ...
      switch (commonItem) {
        ...
        case '💼': {
          const bagPrice = BigInt(this.client.util.getRandomInt(100 + rarity.value, 500 + rarity.value));
    ...
    if (randomNumber <= 0.995) {
      ...
      const price = BigInt(this.client.util.getRandomInt(500 + rarity.value, 750 + rarity.value));
    ...
    const cost = BigInt(this.client.util.getRandomInt(2000, 5000));
  ...
  public exec = async ({ cooldown, interaction, responder }: CommandData) => {
    ...
    userFishingRod.price = BigInt(Math.floor(Number(userFishingRod.price) * (userFishingRod.durability / userFishingRod.maxDurability)));
    ...
    postFishing.item.price *= BigInt(multiplier);
