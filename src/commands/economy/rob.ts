...
export default class Rob extends CommandBase {
  ...
  public exec = async ({ cooldown, interaction, responder, options }: CommandData) => {
    ...
    if (randomNumber < 0.8) {
      if ((userData.balance * 0.05) < 500) robAmount = 500;
      else robAmount = Math.round(userData.balance * 0.05);
      ...
      if (robberStopper) {
        if ((userData.balance * 0.05) < 500) robAmount = 500;
        else robAmount = userData.balance * 0.05;
      ...
      robAmount = Math.round(robbeeData.balance * 0.3);
      ...
      robAmount = Math.round(robbeeData.balance * 0.5);
      ...
      robAmount = Math.round(robbeeData.balance * 0.9);
