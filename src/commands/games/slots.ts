...
export default class Slots extends CommandBase {
  ...
  public exec = async ({ interaction, options, responder }: CommandData) => {
    ...
    if (winnings === 0) {
      ...
      embed.footer = {
        text: `You now have ${this.client.modules.economy.parseInt(userData.balance - amount)}`,
      };
      ...
      embed.footer = {
        text: `You now have ${this.client.modules.economy.parseInt(userData.balance - winnings)}`,
      };
