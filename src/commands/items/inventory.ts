...
export default class Inventory extends CommandBase {
  ...
  public exec = async ({ interaction, responder }: CommandData) => {
    ...
    responder.sendRaw({
      embeds: [{
        ...
        color: responder.colors.default,
        description: thisPage.map(item => `**${item.name}** - *x${item.count}*`).join('\n\n'),
        footer: {
  ...
  private filterInventory(value: string, filteredArray: Item[]) {
    switch (value) {
      case 'oldestItems': {
      ...
      case 'mostValue': {
        filteredArray.sort((itemA, itemB) => itemB.price - itemA.price);
        break;
      }

      case 'leastValue': {
        filteredArray.sort((itemA, itemB) => itemA.price - itemB.price);
        break;
      }
