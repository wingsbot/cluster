...
export default class Sell extends CommandBase {
  ...
  public exec = async ({ interaction, responder, options }: CommandData) => {
    ...
    if (itemId === 'allItems') {
      ...
      totalWingGain += existingItem.price;
      responseMessage = `Successfully sold \`x${existingItem.count} ${existingItem.name}\` for **${this.client.modules.economy.parseInt(totalWingGain)}**`;
