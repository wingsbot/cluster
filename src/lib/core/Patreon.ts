...
export class Patreon {
  ...
  private readonly db: Collection<{ _id: string }>;
  ...
  constructor(client: Shard) {
    this.client = client;
    this.db = this.client.db.collection('patrons');
  ...
  private async fetchPatreon(url?: string) {
    ...
    for (const patron of body.included) {
      ...
      if (!pledgeData.attributes.last_charge_status || pledgeData.attributes.last_charge_status !== 'Paid' || pledgeData.attributes.currently_entitled_amount_cents === 0) {
        updateUsers.push({
          updateOne: {
            filter: { _id: connections.discord.user_id },
            update: {
              $set: {
                ...
                entitledTiers: [],
                patronId: patron.id,
                isDiscordUser: Boolean(connections.discord.user_id),
                amount: pledgeData.attributes.currently_entitled_amount_cents,
              },
            },
            upsert: true,
          },
      ...
      const isExisting = await this.db.findOne({ _id: connections.discord.user_id }) as UserPatronData;
      if (isExisting && isExisting.amount === pledgeData.attributes.currently_entitled_amount_cents) continue;
      ...
      updateUsers.push({
        updateOne: {
          ...
          update: {
            $set: {
              premiumFeatures: Boolean(premiumFeatures),
              patreonBot: Boolean(patreonBot),
              entitledTiers: tiers,
              patronId: patron.id,
              isDiscordUser: Boolean(connections.discord.user_id),
              amount: pledgeData.attributes.currently_entitled_amount_cents,
    ...
    if (updateUsers.length > 0) await this.db.bulkWrite(updateUsers);
  ...
  public async hasPremium(userId: string) {
    ...
    const userData = await this.getData(userId) as UserPatronData;
    if (userData) return userData.premiumFeatures;
